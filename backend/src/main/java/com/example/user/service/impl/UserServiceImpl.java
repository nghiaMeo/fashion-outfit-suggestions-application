...
public class UserServiceImpl implements UserService {
    ...
    public UserProfileResponse getUserProfile(UUID userId) {
        ...
        com.example.social.entity.enums.FriendshipStatus status = null;
        if (!isSelf) {
            status = friendshipRepository.findRelation(currentUser.getId(), userId)
                    .map(com.example.social.entity.Friendship::getStatus)
                    .orElse(null);
        }

        if (targetUser.isPrivateProfile() && !isSelf && status != com.example.social.entity.enums.FriendshipStatus.ACCEPTED) {
